...
public class BookRepository : IBookRepository
{
    ...
    public async Task<List<BookEntity>?> GetBooksAsync(){
        ...
        {
            throw new Exception($"Database error while retrieving books: {ex.Message}", ex);
        }
    }
    ...
    public async Task<BookEntity?> GetBookByIDAsync(string objectId){
        ...
        {
            throw new Exception($"Database error while retrieving book with ID '{objectId}': {ex.Message}", ex);
        }
    }
    ...
    public async Task<string?> CreateBookAsync(BookEntity bookEntity){
        ...
        {
            throw new Exception($"Database error while creating book: {ex.Message}", ex);
        }
    }
    ...
    public async Task<BookEntity> UpdateBookAsync(string objectId, BookEntity bookEntity){
        ...
        {
            throw new Exception($"Database error while updating book with ID '{objectId}': {ex.Message}", ex);
        }
    }
        
     public async Task<DeleteResult> DeleteBookAsync(string objectId){
        try
        {
            return await _bookCollection.DeleteOneAsync<BookEntity>(b => b.ObjectId == objectId);
        }
        catch (MongoException ex)
        {
            throw new Exception($"Database error while deleting book with ID '{objectId}': {ex.Message}", ex);
        }
    }
}